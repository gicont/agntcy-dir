...
func (c *CleanupManager) StartLabelRepublishTask(ctx context.Context) {
	...
	cleanupLogger.Info("Started label republishing task", "interval", RepublishInterval)

	for {
		select {
		case <-ctx.Done():
			cleanupLogger.Info("Label republishing task stopped")

			return
		case <-ticker.C:
			c.republishLocalLabels(ctx)
		}
	}
}
...
// republishLocalLabels republishes all local label mappings to the DHT network
// to ensure they remain discoverable in the DHT.
func (c *CleanupManager) republishLocalLabels(ctx context.Context) {
	cleanupLogger.Info("Starting label republishing cycle")
	...
	for result := range results.Next() {
		...
		if err != nil {
			cleanupLogger.Warn("Failed to pull record for republishing, marking as orphaned", "cid", cid, "error", err)
		...
		for _, label := range labels {
			...
			err = c.server.DHT().PutValue(ctx, enhancedKey, []byte(cid))
			if err != nil {
				cleanupLogger.Warn("Failed to republish enhanced label mapping", "enhanced_key", enhancedKey, "error", err)

				errorCount++
			} else {
				cleanupLogger.Debug("Successfully republished enhanced label mapping", "enhanced_key", enhancedKey)
	...
	cleanupLogger.Info("Completed label republishing cycle",
